...
import { FileText, Upload, BriefcaseBusiness, CheckCircle2, AlertTriangle, LogOut, MapPin, Sun, Moon } from 'lucide-react';
...
export default function CandidateDashboard({ user, darkMode, setDarkMode, onLogout }) {
  ...
  useEffect(() => {
    fetchJobs().then((data) => setJobs(data.jobs || [])).catch(() => setJobs([]));
  }, []);
  ...
  const handleResumeUpload = async (event) => {
    ...
    try {
      ...
      setProfile({
        ...
        experience: `${candidate.experience?.years || 0} years experience`,
        education: candidate.education?.degree || candidate.education?.field || 'Education extracted from resume',
      });
    ...
    <div className={`min-h-screen transition-colors ${darkMode ? 'bg-[#0B0F17] text-[#F3F4F6]' : 'bg-slate-50 text-slate-900'}`}>
      <header className={`flex flex-wrap items-center justify-between gap-4 border-b px-5 py-4 sm:px-8 ${darkMode ? 'border-[#1D2636] bg-[#111622]' : 'border-slate-200 bg-white'}`}>
        <div className="flex items-center gap-3">
          <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-indigo-600 text-sm font-black">R</div>
      ...
      <main className="mx-auto max-w-6xl space-y-6 px-5 py-7 sm:px-8">
        ...
        <div className="grid gap-5 lg:grid-cols-[1.1fr_0.9fr]">
          <section className={`rounded-2xl border p-5 sm:p-6 ${darkMode ? 'border-[#1D2636] bg-[#111622]' : 'border-slate-200 bg-white shadow-sm'}`}>
            ...
            <label className="mt-6 flex cursor-pointer items-center justify-center gap-2 rounded-xl bg-indigo-600 px-4 py-3 text-sm font-bold transition hover:bg-indigo-500"><Upload className="h-4 w-4" />{isUploading ? 'Analyzing resume...' : match ? 'Update resume' : 'Upload resume'}<input type="file" accept=".pdf,.doc,.docx,.txt" onChange={handleResumeUpload} className="hidden" /></label>
          </section>
        ...
        {match && <section className={`rounded-2xl border p-5 sm:p-6 ${darkMode ? 'border-[#1D2636] bg-[#111622]' : 'border-slate-200 bg-white shadow-sm'}`}><div className="flex flex-wrap items-center justify-between gap-3"><div><p className={`text-xs font-bold uppercase tracking-wider ${darkMode ? 'text-gray-500' : 'text-slate-500'}`}>Full AI match details</p><h2 className="mt-1 text-xl font-black">{match.title}</h2><p className={`mt-1 text-sm ${darkMode ? 'text-gray-400' : 'text-slate-600'}`}>{match.company} · {match.department} · {match.location}</p></div><span className="rounded-xl bg-emerald-500/10 px-4 py-2 text-2xl font-black text-emerald-400">{Math.round(match.match.overall_score)}%</span></div><div className="mt-5 grid gap-5 md:grid-cols-3"><ResultGroup icon={CheckCircle2} title="Matched skills" items={match.match.skill_match?.matched_skills || []} color="text-emerald-400" darkMode={darkMode} /><ResultGroup icon={AlertTriangle} title="Skill gaps" items={match.match.skill_match?.missing_skills || []} color="text-amber-400" darkMode={darkMode} /><div><p className={`text-xs font-bold uppercase tracking-wider ${darkMode ? 'text-gray-500' : 'text-slate-500'}`}>Application status</p><p className="mt-3 flex items-center gap-2 text-sm font-bold"><FileText className="h-4 w-4 text-indigo-400" />Not applied yet</p><p className={`mt-3 text-xs ${darkMode ? 'text-gray-400' : 'text-slate-600'}`}>{match.match.recommendation?.recommendation || 'Match reviewed by AI'}</p></div></div></section>}
      </main>
    </div>
  );
}
